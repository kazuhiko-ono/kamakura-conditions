...
test.describe('Phase 9: 風向マップ', () => {
  ...
  test('風向矢印マーカーが地図上に存在する', async ({ page }) => {
    await page.goto('/');
    await expect(page.locator('#status')).toHaveText('取得成功', { timeout: 15000 });
    await expect(page.locator('#wind-map .wind-map-arrow svg')).toBeVisible();
  });

  test('矢印 SVG に rotate が含まれる', async ({ page }) => {
    await page.goto('/');
    await expect(page.locator('#status')).toHaveText('取得成功', { timeout: 15000 });
    const style = await page.locator('#wind-map .wind-map-arrow svg').getAttribute('style');
